...
#'  than one quantile.
#'
#' @param object This is an object of class "zitobitQR", produced by a call to the
#'  zitobitQR function.
...
summary.zitobitQR <- function (object, burnin = 1000, ci = 0.95, ...)
{
  ...
  output$GammaPosterior <- lapply(object$chains, function(a){
    ...
    quantilesL <- apply(a$GammaSample[(burnin+1):numIt, ], 2, stats::quantile, (1-ci)/2)
    quantilesU <- apply(a$GammaSample[(burnin+1):numIt, ], 2, stats::quantile, 1-(1-ci)/2)
