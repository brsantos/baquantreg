...
#'  credible intervals.
#'
#' @param object This is an object of the class "summary.twopartQR",
#' produced by a call to the summary.twopartQR function.
...
plot.summary.twopartQR <- function(object, separate = FALSE, beta = TRUE,
                             sigma = FALSE, gamma = FALSE, ...){
  if (class(object) != "summary.twopartQR")
  ...
  estimates_g <- as.numeric(sapply(object$GammaPosterior, function(a) a[,2]))
  lowerQ_g <- as.numeric(sapply(object$GammaPosterior, function(a) a[,3]))
  upperQ_g <- as.numeric(sapply(object$GammaPosterior, function(a) a[,4]))
