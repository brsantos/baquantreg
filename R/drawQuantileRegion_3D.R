...
drawQuantileRegion_3D <- function(model, datafile, response,
  ...
  pointsPlot <-  lapply(1:ntaus, function(a){
    ...
    points_inside <- lapply(1:ngridpoints, function(aaa){
      checkPoints_values_aux <- checkPoints_values[, , aaa]
      y1_ind <- which(rowSums(checkPoints_values_aux) > 0)
      if(length(y1_ind) > 0){

        y1_inside <- seqY1[y1_ind]

        sub_checkPoints <- checkPoints_values_aux[y1_ind, ]

        if(!is.null(dim(sub_checkPoints))){
          y2_inside_max <- apply(sub_checkPoints, 1, function(a)
            seqY2[max(which(a == 1))])
          y2_inside_min <- apply(sub_checkPoints, 1, function(a)
            seqY2[min(which(a == 1))])
        } else {
          y2_inside_max <- seqY2[max(which(sub_checkPoints == 1))]
          y2_inside_min <- seqY2[min(which(sub_checkPoints == 1))]
        }
      }
      else {
        y1_inside <- NA
